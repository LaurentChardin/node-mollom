 ...
 * https://github.com/laurentchardin/node-mollom/blob/master/LICENSE
 */
...
function Mollom(options) {
    ...
    var defaults = {
        server: 'rest.mollom.com', // dev.mollom.com
        secure: false,
        consumer_key: null,
    ...
    if (_.isBoolean(options.use_test) && options.use_test) {
        defaults.server = 'dev.mollom.com';
    }

    this._options = _.extend(defaults, options);

    // Default options used for HTTP calls
    this._reqOptions = {
        headers: {
    ...
    // Setup *request* object defaults
    this._req = req.defaults(this._reqOptions);

    this.server = (this._options.secure) ? 'https://' : 'http://';
    this.server += this._options.server;

    // If debug is set, additional configuration
    if (this._options.debug) {
        // request debug mode
        require('request-debug')(req, function(type, data) {
...
Mollom.prototype = {
    ...
    _query: function (method, path, data, options, callback) {
        ...
        var max_attempts = this._options.requestMaxAttempts;
        ...
        if (method == 'POST') {
            ...
            if (options !== null && options.noOAuth) {
                this._req.post(_.extend(body, {url: url}), callback);
            }
            else {
                this._req.post(_.extend(body, {url: url, oauth: oauth}), callback);
            }
        }
        else if (method == 'GET') {
            // is oAuth configured ?
            this._req.get({url: url, oauth: oauth}, callback);
        }
        else {
            // todo
        }
     ...
     * @param callback
     */
    createCaptcha: function (data, callback) {
        //@todo check data format
        if (typeof callback !== 'function') {
            throw "FAIL: INVALID CALLBACK.";
        }
...
var exports = module.exports = Mollom;
